...
```{r, eval = FALSE}
debits_plusieurs_stations <- menv_bh_htr_tod(stations_id = stations_id) %>% 
  mutate(date_obs = lubridate::ymd_hms(date_obs)) %>%  # format de la date
...
ggplot(data = debits_plusieurs_stations, aes(x = date_obs, y = resultat_obs)) +
  ...
  facet_wrap( ~ str_wrap(libelle_site, 25), # renvoi à la ligne si les noms des stations sont longs
              scales = 'free_y', # l'échelle verticale est différente pour chaque station
